...
const subQueues = Object.keys(EventHandler).map((key) => {
  const k = `${RedisKeys.SteamGiftAppealQueue}_${key}`
  KeyMap[k] = key
  return k
})
...
async function taskRun(app, message: string, channel, cb = null) {
  ...
  // 当前进程
  let _data = await redis.master.client.lPop(
    RedisKeys.SteamGiftAppealQueue as any
  )
  if (!_data) return
  let data = JSON.parse(_data) as ChannelData
  const qKey = `${RedisKeys.SteamGiftAppealQueue}_${data.key}`
  // 分发到子任务队列
...
export default async function init(app) {
  ...
  ready = true
  const len =
    (await redis.master.client.lLen(RedisKeys.SteamGiftAppealQueue as any)) || 0
  ...
  setInterval(async () => {
    ...
    const len =
      (await redis.master.client.lLen(RedisKeys.SteamGiftAppealQueue as any)) ||
      0
    // logger.info('队列数', len)
    const store = taskStore.get()
    ...
    subQueues.map(async (qkey) => {
      ...
      Log.task.info(
        `process ${process.pid}  ${qkey} 当前进行中任务数`,
        store[taskKey],
        '剩余',
        len
      )
  ...
  await redis.master.subscribe(
    CHANNEL_KEY,
    async (message: string, channel) => {
